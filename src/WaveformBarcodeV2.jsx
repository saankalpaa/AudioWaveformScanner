...
const decodeFromHeights = (heights) => {
  ...
  if (calculatedCrc !== checksum) {
    return { error: `CRC mismatch`, data };
  }

  return { data, valid: true };
};

// In-memory URL storage (since localStorage not available in artifacts)
...
function WaveformBarcodeV2() {
  ...
  const decodeCanvas = (canvas) => {
    ...
    return decodeFromHeights(finalHeights);
  };
                ...
                <div className="space-y-3">
                  <div className="relative bg-black rounded-lg overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <div className="border-2 border-green-400 w-4/5 h-16 rounded opacity-60" />
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button
